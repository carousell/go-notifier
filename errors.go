...
// ErrorExt is the interface that defines an error, any ErrorExt implementors can use and override errors and notifier package
...
// ShouldNotify implements notifier.NotifyExt
func (c *customError) ShouldNotify() bool {
	return c.shouldNotify
}

// Notified implements notifier.NotifyExt
...
// WrapWithSkipAndStatus wraps an existing error and appends stack information
// if it does not exist skipping the number of function on the stack along with GRPC status
